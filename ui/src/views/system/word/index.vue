<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="停用词" prop="stopWord">
        <el-input
          v-model="queryParams.stopWord"
          placeholder="请输入停用词"
          clearable
    ...
    <el-table v-loading="loading" :data="wordList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="停用词" align="center" prop="stopWord" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="停用词" prop="stopWord">
          <el-input v-model="form.stopWord" placeholder="请输入停用词" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        stopWord: [
          { required: true, message: "停用词不能为空", trigger: "blur" }
        ],
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除停用语料库编号为"' + ids + '"的数据项？').then(function() {
        return delWord(ids)
      }).then(() => {
        this.getList()
